...
# ---- PyTorch API compatibility patch (torch 2.1.x vs newer) ----
# Newer SAM3 code may call torch.compiler.is_dynamo_compiling(), which is absent in torch==2.1.x.
if not (hasattr(torch, "compiler") and hasattr(torch.compiler, "is_dynamo_compiling")):
    if not hasattr(torch, "compiler"):
        torch.compiler = types.SimpleNamespace()  # create namespace if missing
