...
# --- SDPA backend selection: new API (torch.nn.attention) with fallback to old API (torch.backends.cuda) ---
import torch
import inspect
from contextlib import nullcontext

try:
    # Newer PyTorch (has torch.nn.attention)
    from torch.nn.attention import sdpa_kernel, SDPBackend
except ModuleNotFoundError:
    # Older PyTorch: emulate torch.nn.attention.sdpa_kernel using torch.backends.cuda.sdp_kernel
    ...
    SDPBackend = getattr(torch.backends.cuda, "SDPBackend", _SDPBackend)

    def sdpa_kernel(backends, set_priority: bool = False):
        """
        Fallback for torch.nn.attention.sdpa_kernel:
        returns a valid context manager for selecting SDPA backends.
        """
        if backends is None:
            return nullcontext()
        ...
        def _name(b):
            s = str(b)
            return s.split(".")[-1]  # "SDPBackend.FLASH_ATTENTION" -> "FLASH_ATTENTION"

        bset = {_name(b) for b in backends}

        kwargs = dict(
            enable_flash=("FLASH_ATTENTION" in bset),
            enable_math=("MATH" in bset),
            enable_mem_efficient=("EFFICIENT_ATTENTION" in bset),
        )

        # torch==2.1.x often has no enable_cudnn; only pass if supported
        try:
            sig = inspect.signature(torch.backends.cuda.sdp_kernel)
            if "enable_cudnn" in sig.parameters:
                kwargs["enable_cudnn"] = ("CUDNN_ATTENTION" in bset)
        except Exception:
            pass

        # Return the actual context manager
        try:
            return torch.backends.cuda.sdp_kernel(**kwargs)
        except TypeError:
            # If enable_cudnn is not supported, retry without it
            kwargs.pop("enable_cudnn", None)
            return torch.backends.cuda.sdp_kernel(**kwargs)
# --- end ---
...
class SAM3VLBackbone(nn.Module):
    ...
    def _forward_image_no_act_ckpt(self, samples):
        ...
        # sam3_features: [1, 256, 288, 288], [1, 256, 144, 144], [1, 256, 72, 72], [1, 256, 32, 32]
        if self.scalp > 0:
            ...
            # sam3_features: [1, 256, 144, 144], [1, 256, 72, 72], [1, 256, 32, 32]
            # sam3_pos: [1, 256, 144, 144], [1, 256, 72, 72], [1, 256, 32, 32]
